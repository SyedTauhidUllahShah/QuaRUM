...
def main():
    ...
    settings.set(
        "output", "diagram_style", "vibrant"
    )  # Use vibrant style for the diagram
    ...
    result = pipeline.execute(
        file_path=args.file, domain_description=args.domain, output_dir=args.output_dir
    )
    ...
    if result.success:
        ...
        print(
            f"Relationships identified: {result.metrics.get('final_relationship_count', 0)}"
        )
