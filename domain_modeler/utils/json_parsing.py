...
class JSONExtractor:
    ...
    def extract_type(
        ...
        if not isinstance(result, expected_type):
            self.last_error = f"Extracted JSON is not of expected type {expected_type.__name__}"
            logger.warning(self.last_error)
            return None
    ...
    def extract_with_schema(
        ...
        if missing_keys:
            self.last_error = f"Extracted JSON is missing required keys: {', '.join(missing_keys)}"
            logger.warning(self.last_error)
            return None
