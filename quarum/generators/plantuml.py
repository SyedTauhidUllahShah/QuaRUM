...
class PlantUMLGenerator:
    ...
    def generate(self, style: Optional[str] = None) -> str:
        ...
        self._add_entity_package(
            lines, "AbstractClasses", abstract_classes, "abstract class"
        )
    ...
    def _add_entity_package(
        ...
        for entity in sorted(entities, key=lambda x: x.name):
            # Get stereotypes
            stereotypes = [
                stereotype_map[s] for s in entity.stereotypes if s in stereotype_map
            ]
    ...
    def _add_relationships(self, lines: list[str]) -> None:
        ...
        for rel in sorted(self.code_system.relationships, key=lambda r: r.id):
            ...
            if rel.relationship_type == CSLRelationshipType.IS_A:
                ...
                CSLRelationshipType.DEPENDS_ON,
                CSLRelationshipType.USES,
            ]:
                lines.append(
                ...
                target_mult = rel.multiplicity.get("target", "*")
                lines.append(
                    f'{source_name} "{source_mult}" -- "{target_mult}"'
                    + f' {target_name} : {rel.association_name or ""}'
                )
