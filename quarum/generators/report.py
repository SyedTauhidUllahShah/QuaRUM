...
class ReportGenerator:
    ...
    def generate_markdown(self, include_metrics: bool = True) -> str:
        ...
        if include_metrics:
            ...
            report.append(
                f"- **Implementation relationships:** {metrics['implementation']}"
            )
        ...
        for code in sorted(self.code_system.codes.values(), key=lambda x: x.name):
            ...
            if code.attributes:
                ...
                for attr in code.attributes:
                    report.append(
                        f"  - {attr['visibility']} {attr['name']}: {attr['type']}"
                    )
            ...
            if code.methods:
                ...
                for method in code.methods:
                    report.append(
                        f"  - {method['visibility']} {method['name']}{method['signature']}"
                    )
        ...
        for rel in sorted(self.code_system.relationships, key=lambda r: r.id):
            ...
            if (
                not source
                or not target
                or self._is_banned_term(source.name)
                or self._is_banned_term(target.name)
            ):
                continue
            ...
            report.append(
                f"- **Multiplicity:** {rel.multiplicity['source']} : {rel.multiplicity['target']}"
            )
    ...
    def save_report(
        self, output_path: str, format: str = "markdown", include_metrics: bool = True
    ) -> None:
        """
    ...
    def _is_banned_term(self, name: str) -> bool:
        ...
        """
        banned_words = set(
            ["system", "component", "generic", "entity", "item", "object", "module"]
        )
        nl = name.strip().lower()
    ...
    def generate_metrics_report(self) -> str:
        ...
        inheritance_percent = (
            (metrics['inheritance'] / metrics['relationships'] * 100)
        ...
        relationships_per_entity = (
            metrics['relationships'] / metrics['total_entities']
