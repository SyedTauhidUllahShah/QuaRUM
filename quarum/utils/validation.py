...
class ModelValidator:
    ...
    def _check_entity_names(self) -> None:
        ...
        for code_id, code in self.code_system.codes.items():
            ...
            if not re.match(r'^[a-zA-Z0-9_\s]+$', code.name):
                self.issues.append(
                    f"Entity '{code.name}' contains non-alphanumeric characters"
                )
    ...
    def _check_orphaned_entities(self) -> None:
        ...
        for code_id, code in self.code_system.codes.items():
            if code_id not in connected_entities:
                ...
                self.warnings.append(
                    f"Orphaned entity: '{code.name}' has no relationships"
                )
    ...
    def _check_relationship_validity(self) -> None:
        ...
        for rel in self.code_system.relationships:
            ...
            if rel.source_code_id not in self.code_system.codes:
                self.issues.append(
                    f"Relationship {rel.id} references non-existent source entity: "
                    + f"{rel.source_code_id}"
                )
                continue

            if rel.target_code_id not in self.code_system.codes:
                self.issues.append(
                    f"Relationship {rel.id} references non-existent target entity: "
                    + f"{rel.target_code_id}"
                )
                continue
            ...
            if source.is_enumeration:
                if rel.relationship_type not in [
                    ...
                    self.issues.append(
                        f"Enumeration '{source.name}' has invalid relationship type: "
            ...
            if target.is_enumeration:
                if rel.relationship_type not in [
                    ...
                    self.issues.append(
                        f"Enumeration '{target.name}' has invalid relationship type: "
                        + f"{rel.relationship_type.value}"
                    )
    ...
    def _check_inheritance_cycles(self) -> None:
        ...
        for code_id in self.code_system.codes:
            ...
            if self._has_inheritance_cycle(code_id, visited):
                self.issues.append(
                    "Inheritance cycle detected involving entity: "
                    + f"{self.code_system.codes[code_id].name}"
                )
    ...
    def _has_inheritance_cycle(self, code_id: str, visited: set[str]) -> bool:
        ...
        for rel in self.code_system.relationships:
            if (
                rel.source_code_id == code_id
                and rel.relationship_type == CSLRelationshipType.IS_A
            ):
    ...
    def _check_interface_implementations(self) -> None:
        ...
        for code_id, code in self.code_system.codes.items():
            if code.is_interface:
                ...
                for rel in self.code_system.relationships:
                    if (
                        rel.target_code_id == code_id
                        and rel.relationship_type == CSLRelationshipType.IMPLEMENTATION
                    ):
                ...
                if not has_implementation:
                    self.warnings.append(
                        f"Interface '{code.name}' has no implementations"
                    )
    ...
    def _check_relationship_multiplicities(self) -> None:
        ...
        for rel in self.code_system.relationships:
            ...
            if (
                rel.source_code_id not in self.code_system.codes
                or rel.target_code_id not in self.code_system.codes
            ):
                continue
            ...
            if rel.relationship_type in [
                ...
                if (
                    rel.multiplicity["source"] != "1"
                    or rel.multiplicity["target"] != "1"
                ):
                    self.issues.append(
        ...
        for code_id, code in self.code_system.codes.items():
            ...
            if not re.match(r'^[a-zA-Z0-9_\s]+$', code.name):
                ...
                self.fixes_applied.append(
                    "Removed invalid characters from entity name: "
                    + f"'{original_name}' → '{code.name}'"
                )
        ...
        for rel in self.code_system.relationships:
            ...
            if source.is_enumeration or target.is_enumeration:
                if rel.relationship_type not in [
                    ...
                    relationships_to_change.append(
                        (rel, CSLRelationshipType.ASSOCIATION)
                    )
    ...
    def _break_inheritance_cycle(self, cycle: list[str]) -> None:
        ...
        for i in range(len(cycle) - 1):
            ...
            for rel in self.code_system.relationships:
                if (
                    rel.source_code_id == source_id
                    and rel.target_code_id == target_id
                    and rel.relationship_type == CSLRelationshipType.IS_A
                ):
    ...
    def _fix_interface_implementations(self) -> None:
        ...
        for _, code in self.code_system.codes.items():
            if code.is_interface:
