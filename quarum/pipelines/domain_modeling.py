...
class DomainModelingPipeline(Pipeline):
    ...
    def _setup_components(self) -> None:
        ...
        self.vector_embeddings = VectorEmbeddings(api_key=self.api_key)
    ...
    def setup(
        ...
        try:
            ...
            self.add_message(
                "Setup complete. Core entities: "
                + f"{', '.join(self.code_system.core_domain_entities[:5])}"
            )
    ...
    def _load_document(self, file_path: str) -> bool:
        ...
        try:
            ...
            if len(self.documents) > max_chunks:
                self.add_warning(
                    f"Document contains {len(self.documents)} chunks, limiting to {max_chunks}"
                )
...
Ensure JSON is valid and focuses on business domain concepts."""
            ...
            for attempt in range(3):
                try:
                    ...
                    if result:
                        # Store core entities
                        self.code_system.core_domain_entities = result.get(
                            "core_entities", []
                        )
                        ...
                        self.code_system.domain_attributes = result.get(
                            "attributes", {}
                        )
                        self.code_system.domain_operations = result.get(
                            "operations", {}
                        )
                    ...
                    self.add_warning(
                        f"Domain analysis attempt {attempt+1} failed: {str(e)}"
                    )
    ...
    def execute(
        ...
        try:
            ...
            # Add phase result
            self.add_metric(
                "entities_extracted", open_result.metrics.get("final_entity_count", 0)
            )
            ...
            self.add_metric(
                ...
                axial_result.metrics.get("relationship_count", 0),
            )
            ...
            self.add_metric(
                "final_entity_count",
                selective_result.metrics.get("final_class_count", 0),
            )
            self.add_metric(
                "final_relationship_count",
                selective_result.metrics.get("final_relationship_count", 0),
            )
            ...
            if not validation_success:
                self.add_warning(
                    "Model validation found issues that could not be automatically fixed"
                )
            ...
            plantuml_file = os.path.join(
                output_dir, f"{self.domain_name.lower()}_model_{timestamp}.puml"
            )
            ...
            report_file = os.path.join(
                output_dir, f"{self.domain_name.lower()}_report_{timestamp}.md"
            )
