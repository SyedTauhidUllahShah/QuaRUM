...
class CodeSystem:
    ...
    def add_code(self, code: Code) -> str:
        ...
        for eid, existing_code in self.codes.items():
            ...
            if (
                existing_code.name.lower() == code.name.lower()
                or SequenceMatcher(
                    None, existing_code.name.lower(), code.name.lower()
                ).ratio()
                > 0.9
            ):

                # Merge attributes (ensuring uniqueness)
                existing_code.attributes = list(
                    set(
                        json.dumps(attr, sort_keys=True)
                        for attr in existing_code.attributes + code.attributes
                    )
                )
                existing_code.attributes = [
                    json.loads(attr) for attr in existing_code.attributes
                ]

                # Merge methods (ensuring uniqueness)
                existing_code.methods = list(
                    set(
                        json.dumps(meth, sort_keys=True)
                        for meth in existing_code.methods + code.methods
                    )
                )
                existing_code.methods = [
                    json.loads(meth) for meth in existing_code.methods
                ]

                # Merge stereotypes
                existing_code.stereotypes = list(
                    set(existing_code.stereotypes + code.stereotypes)
                )
                ...
                existing_code.confidence = max(
                    existing_code.confidence, code.confidence
                )
                ...
                if code.is_enumeration:
                    ...
                    existing_code.enum_values = list(
                        set(existing_code.enum_values + code.enum_values)
                    )
    ...
    def add_relationship(self, relationship: CodeRelationship) -> bool:
        ...
        if (
            relationship.source_code_id not in self.codes
            or relationship.target_code_id not in self.codes
        ):
            return False
        ...
        if (
            relationship.relationship_type == CSLRelationshipType.IMPLEMENTATION
        ...
        for r in self.relationships:
            if (
                r.source_code_id == relationship.source_code_id
                and r.target_code_id == relationship.target_code_id
    ...
    def calculate_relevance_scores(self) -> None:
        ...
        for _, code in self.codes.items():
            ...
            relation_count = len(code.incoming_relationships) + len(
                code.outgoing_relationships
            )
    ...
    def get_codes_by_stereotype(self, stereotype: str) -> list[Code]:
        ...
        """
        return [code for code in self.codes.values() if stereotype in code.stereotypes]
    ...
    def get_relationships_by_type(
        self, rel_type: CSLRelationshipType
